...
class TestSearchPipelineBasic:
    ...
    def search_pipeline(self, mock_kyrodb_router, mock_embedding_service):
        ...
        return SearchPipeline(
            kyrodb_router=mock_kyrodb_router, embedding_service=mock_embedding_service
        )
    ...
    async def test_basic_search_flow(self, search_pipeline):
        ...
        request = SearchRequest(
            goal="Test query", customer_id="test_customer", collection="failures", k=5
        )
...
class TestSearchPipelineLLMIntegration:
    ...
    @pytest.mark.asyncio
    async def test_llm_rejects_semantic_negation(
        self, mock_kyrodb_with_results, mock_embedding_service
    ):
        ...
        advanced_matcher = AdvancedPreconditionMatcher(
            openrouter_api_key="test_key", enable_llm=True
        )
    ...
    @pytest.mark.asyncio
    async def test_llm_accepts_compatible_query(
        self, mock_kyrodb_with_results, mock_embedding_service
    ):
...
class TestPerformanceMetrics:
    ...
    async def test_stats_tracking_without_llm(self):
        ...
        pipeline = SearchPipeline(kyrodb_router=router, embedding_service=embedding)

        # Perform search
        request = SearchRequest(
            goal="Test query for stats", customer_id="test", collection="failures", k=5
        )
        await pipeline.search(request)
    ...
    async def test_stats_tracking_with_llm(self):
        ...
        advanced_matcher = AdvancedPreconditionMatcher(openrouter_api_key="test", enable_llm=True)
