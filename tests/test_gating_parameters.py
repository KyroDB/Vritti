...
@pytest.mark.asyncio
async def test_proposed_action_included_in_skill_hints(
    gating_service, mock_search_pipeline, mock_kyrodb_router
):
    ...
    mock_search_pipeline.search_with_embedding = AsyncMock(
...
async def test_environment_mismatch_warning_in_block(gating_service, mock_search_pipeline):
    ...
    search_result = SearchResult(
        episode=episode, scores={"similarity": 0.95, "precondition": 0.8}, rank=1
    )
...
async def test_environment_match_in_rewrite(gating_service, mock_search_pipeline):
    ...
    search_result = SearchResult(
        episode=episode, scores={"similarity": 0.85, "precondition": 0.6}, rank=1
    )
...
def test_check_environment_match_with_matches(gating_service):
    ...
    current_state = {"os": "linux", "version": "kubernetes-1.28", "cloud": "aws"}

    # Should match when factor is present
    assert gating_service._check_environment_match(current_state, ["linux", "ubuntu"]) is True

    # Should match when at least one factor is present
    assert gating_service._check_environment_match(current_state, ["kubernetes"]) is True

    # Should not match when no factors are present
    assert gating_service._check_environment_match(current_state, ["windows", "azure"]) is False
...
def test_check_environment_match_case_insensitive(gating_service):
    ...
    assert gating_service._check_environment_match(current_state, ["linux"]) is True
