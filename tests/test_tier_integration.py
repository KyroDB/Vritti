...
class TestTieredIntegration:
    ...
    async def test_capture_with_tier_override_cheap(self):
        ...
        episode = await pipeline.capture_episode(episode_data, tier_override=ReflectionTier.CHEAP)
        ...
        call_kwargs = reflection_service.generate_reflection.call_args[1]
        assert (
            call_kwargs.get("tier") == ReflectionTier.CHEAP
        ), "CHEAP tier override should be passed"
    ...
    async def test_capture_with_tier_override_premium(self):
        ...
        episode = await pipeline.capture_episode(episode_data, tier_override=ReflectionTier.PREMIUM)
        ...
        call_kwargs = reflection_service.generate_reflection.call_args[1]
        assert (
            call_kwargs.get("tier") == ReflectionTier.PREMIUM
        ), "PREMIUM tier override should be passed"
