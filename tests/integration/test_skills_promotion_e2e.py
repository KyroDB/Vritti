        ...
        try:
            ...
            for episode in inserted_episodes:
                with contextlib.suppress(Exception):
                    await kyrodb_router.delete_episode(episode.episode_id, customer_id, "failures")
...
""",
    ...
    async def test_skill_feedback_updates_stats(
        ...
        try:
            ...
            assert (
                updated_skill.usage_count == 3
            ), f"Usage count should be 3, got {updated_skill.usage_count}"
            assert (
                updated_skill.success_count == 2
            ), f"Success count should be 2, got {updated_skill.success_count}"
            assert (
                updated_skill.failure_count == 1
            ), f"Failure count should be 1, got {updated_skill.failure_count}"

            # Verify success rate: 2/3 = 0.666...
            expected_rate = 2 / 3
            assert (
                abs(updated_skill.success_rate - expected_rate) < 0.01
    ...
    async def test_customer_isolation_for_skills(
        ...
        try:
            ...
            assert skill_b is None, "Customer B should not promote - A's episodes should not count"

        finally:
            # Cleanup
            for episode in inserted_episodes_a:
                with contextlib.suppress(Exception):
                    await kyrodb_router.delete_episode(episode.episode_id, customer_a, "failures")

            for episode in inserted_episodes_b:
                with contextlib.suppress(Exception):
                    await kyrodb_router.delete_episode(episode.episode_id, customer_b, "failures")
...
class TestSkillsInGating:
    ...
    async def test_gating_returns_relevant_skills(
        ...
        skill_embedding = embedding_service.embed_text(f"{skill.name} {skill.docstring}")
