...
def settings_with_api_key() -> Settings:
    ...
    return Settings(
        ...
        llm=LLMConfig(openrouter_api_key="sk-test-key-12345"),  # Valid key
    )
...
def settings_without_api_key() -> Settings:
    ...
    return Settings(
        ...
        llm=LLMConfig(openrouter_api_key=""),  # No key
    )
...
def test_validate_configuration_with_api_key(settings_with_api_key, caplog):
    ...
    assert any(
        "Non-standard text embedding dimension: 200" in record.message for record in caplog.records
    )

    # Should NOT have warning for missing API key
    assert not any("LLM API key not configured" in record.message for record in caplog.records)
...
def test_validate_configuration_without_api_key(settings_without_api_key, caplog):
    ...
    assert any("LLM API key not configured" in record.message for record in caplog.records)
...
def test_validate_configuration_all_warnings(settings_all_nonstandard, caplog):
    ...
    assert any(
        "Non-standard image embedding dimension: 600" in record.message for record in caplog.records
    )
...
def test_validate_configuration_no_warnings():
    ...
    settings = Settings(
        ...
        llm=LLMConfig(openrouter_api_key="sk-test-valid-key-12345"),  # Valid key
    )
