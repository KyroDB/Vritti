...
class GoalParser:
    ...
    def _extract_time_condition(self, goal: str) -> TimeCondition | None:
        ...
        for pattern, operator in self.TIME_PATTERNS:
            ...
            if match:
                ...
                return TimeCondition(operator=operator, value=value, unit=unit, negated=negated)
    ...
    def goals_compatible(
        self, goal1: StructuredGoal, goal2: StructuredGoal, strict: bool = False
    ) -> bool:
        """
        Check if two goals are compatible.

        In non-strict mode, this is a fast heuristic pre-filter before expensive
        LLM validation. In strict mode, this requires exact structural equality.

        Args:
            goal1: First goal
            goal2: Second goal
            strict: If True, require exact match of all StructuredGoal fields

        Returns:
            True if goals appear compatible, False if obviously incompatible
        """
        if strict:
            return goal1.model_dump(mode="python") == goal2.model_dump(mode="python")
        ...
        if goal1.time_condition and goal2.time_condition:
            ...
            if tc1.operator == tc2.operator and tc1.negated != tc2.negated:
                logger.debug("Goals incompatible: time condition negation mismatch")
                return False
